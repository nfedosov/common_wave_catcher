function [sensor_waves, strt] = blob_on_sensor(cortex,  PARAMS, G, strt,speed,nspoints, third_vector, dr0)
...
% 
    ...
    for i = t
       if nspoints == 51
           wave((i+1),:) = exp(-((n-26)*1.8/51).^2)*sin(2*pi*i/102);
       else
           wave((i+1),:) = exp(-((n-nspoints/2)*1.8/51).^2)*sin(2*pi*i/102);%sin(2*pi*(n+i-51)/51).*exp(-((n-20-51+i)/20).^2);
       end
    end%wave - t (0 ... T) x 
    if PARAMS.draw_wave
       figure
